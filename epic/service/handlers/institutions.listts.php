...
function institutions_listts_handler($params) {
  ...
  $query = 'SELECT *, UNIX_TIMESTAMP(`value_to_date`) AS uvtd, UNIX_TIMESTAMP(`value_from_date`) AS uvfd FROM `'.$db_name.'`.`institutions_history`.WHERE UNIX_TIMESTAMP(`date`) <= '.$ts.' ORDER BY `date` ASC';
  $hist = $db->Query($query);
