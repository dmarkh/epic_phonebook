...
RCLIENT.Phonebook = function () {
    ...
    this.service_url = 'https://phonebook.sdcc.bnl.gov/epic_admin/service/index.php';
    this.public_url = 'https://phonebook.sdcc.bnl.gov/epic/client/';
...
RCLIENT.Phonebook.prototype = {
	...
	export_excel_board: function() {
		var service = this;
		var url = service.service_url+'?q=/members/excelx_board/date:'+Date.now();
		location.href = url;
	},
	...
	display_statistics: function() {
		...
		var num_accel = 0, num_theory = 0, num_experiment = 0, num_support = 0, num_other = 0, num_snc = 0;
		...
		for (var i in service.members) {
            ...
            else if (service.members[i]['fields'][89] == 'c') { num_snc += 1; }
            else { num_other += 1; }
		}
		...
		var selects = '<p>I. ePIC: <ul><li><b>'+num_mem+'</b> members</li><li><b>'+num_inst+'</b> institutions</li><li><b>'+num_cntr+'</b> countries (<b>'+num_reg+'</b> world regions)</li></ul> </p>';
            selects += '<p>Experiment Scientists: <b>' + num_experiment + '</b>, Theory Scientists: <b>' + num_theory + '</b>, Accelerator Scientists: <b>' + num_accel + '</b><br>, Computer Scientists: <b>'+ num_snc +'</b>, Support: <b>' + num_support + '</b>, Other: <b>'+num_other+'</b></p>';
	...
	display_member_fields: function() {
	  ...
	  geocode_locate_address: function(address) {
			...
			var req = 'https://photon.komoot.io/api/?q='+encodeURIComponent(address);
    ...
    geocode_locate_address_old: function(address) {
			var req = 'https://nominatim.openstreetmap.org/?q='+encodeURIComponent(address)+'&format=json&limit=1';
      var res = jQuery.parseJSON( jQuery.ajax({ type: "GET", url: req, async:false }).responseText );
			if ( Array.isArray( res ) && res.length == 1 && res[0].lat && res[0].lon ) {
            return ( { lat: res[0].lat, lon: res[0].lon, desc: res[0].display_name } );
			}
			console.log( req );
      return undefined;
    },
    ...
    display_worldmap: function() {
        ...
        $('#worldmapdialog').dialog({
            autoOpen: true,
            height: Math.min( 768, sch * 0.8),
            width: Math.min( 1024, scw * 0.8),
            modal: true,
            buttons: {
                "Close": function() {
                    $( this ).dialog( "close" );
        ...
        service.map.addLayer(Esri_NatGeoWorldMap);
        ...
        var osm = new L.TileLayer('http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {minZoom: 1, maxZoom: 20, attribution: ''});

        service.map.addControl(new L.Control.Layers( {
        ...
        for (var i in service.institutions) {
            ...
            desc += '<div style="display: inline-block; color: #1D78C8; font-size: 16px; font-family: verdana;" onClick="client.display_institution_details('+i+');">'+inst['fields'][1]+'</nobr></div></nobr><hr style="margin: 0; padding: 0; color: #CCC;"><span style="font-family: verdana; font-size: 14px;">';
			...
			try {
				...
				console.log('bad inst: ' + inst['fields'][1] );
				console.log( e );
			}
        }
    },

    display_geocode_dialog: function(pt, desc, search, ilat, ilon ) {
        var service = this;
        $('#geocodedialog').remove();
        $('body').append('<div id="geocodedialog"><table style="width: 850px;"><tr><td style="width: 500px;"><div id="geocodemap" style="height: 500px; width: 500px; display: block;"></div></td><td><input type="text" id="geocodeaddress" style="width: 100%;" readonly><br><br><b>OLD</b> Lat/Lon Coordinates</br><input type="text" id="geocode_lat_old" style="width: 100%;" readonly><br><input type="text" id="geocode_lon_old" style="width: 100%;" readonly><br><br><b>NEW</b> Lat/Lon Coordinates:<br><input type="text" id="geocode_lat_new" style="width: 100%;" readonly><br><input type="text" id="geocode_lon_new" style="width: 100%;" readonly><br></td></tr><table></div>');
        ...
        $('#geocodedialog').dialog({
            ...
            modal: true,
            buttons: {
                "Apply new coordinates": function() {
        ...
        $('#geocode_lat_old').val(ilat || "");
        $('#geocode_lon_old').val(ilon || "");
        $('#geocode_lat_new').val(pt[0] || "");
        $('#geocode_lon_new').val(pt[1] || "");

        var map = new L.Map('geocodemap', { center: new L.LatLng(pt[0], pt[1]), zoom: 2 });
		var Esri_NatGeoWorldMap = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/NatGeo_World_Map/MapServer/tile/{z}/{y}/{x}', {
			attribution: '',
			maxZoom: 16
		});
		map.addLayer(Esri_NatGeoWorldMap);
        map.addControl(new L.Control.Layers( { 'Geo World Map': Esri_NatGeoWorldMap }, {}));
	...
	display_filter_members: function() {
		...
		if ($("#filter-members-dialog").length > 0) {
			...
			<option value="notempty" selected=selected>is not empty</option>\
			</select>\
			...
			$('#filter-members-button').off('click').on('click', function() {
			  ...
			  if ( mdata.length > 0 ) {
                ...
                $('<button id="export-xls-filtered-' + service.tabCounter + '" style="margin-right: 20px;"><img src="images/icons/excel.png" border=0 style="vertical-align: middle;"> Export to Excel</button>')
                  ...
                  .on('click', function() {
                    var wb = new Workbook(), ws = sheet_from_array_of_arrays(mdata), ws_name = "ePIC-members-";
...
*/
	...
	display_filter_institutions: function() {
		var service = this;
		if ($("#filter-institutions-dialog").length > 0) {
			$('#filter-institutions-dialog').dialog('open');
		} else {
			var search_form = '<div id="filter-institutions-dialog" title="Filter Institutions">';
			search_form += '<fieldset><legend>Filters</legend><p><input type="radio" name="filter_andor" class="filter_andor" value="0" checked=checked id="filter_andor1"><label for="filter_andor1">Match all criteria</label>';
			search_form += '<input type="radio" name="filter_andor" class="filter_andor" value="1" id="filter_andor2"><label for="filter_andor2">Match any criteria</label></p>';

			search_form += '<p class="institution_filter_item"><select name="filter-institutions-field" id="filter-institutions-field">';
	        for (var j in service.institutions_fields) {
                var hint = [], tmp;
                if ( service.institutions_fields[j]['options'] && service.institutions_fields[j]['options'].length > 0 ) {
                  tmp = service.institutions_fields[j]['options'].split(',');
			...
			search_form += '</select>';
			search_form += '<select name="filter-institutions-filter-type" id="filter-institutions-filter-type">\
			<option value="equals">equals to</option>\
			<option value="notequals">does not equal to</option>\
			<option value="contains">contains</option>\
			<option value="notcontains">does not contain</option>\
			...
			<input id="filter-institutions-input" type="text" name="cvalue">\
			<button data-op="+" class="btn_click_filter">+</button> <button data-op="-" class="btn_click_filter">-</button>\
			</fieldset>\
			<table width="100%"><tr><td valign="top" width="50%">\
			<fieldset><legend>Display Fields</legend>';

            search_form += '<p class="institution_display_item"><select name="filter-institutions-field-display" id="filter-institutions-field-display">';
            for (var j in service.institutions_fields) {
                search_form += '<option value="'+service.institutions_fields[j].id+'_i" '+(j == 1 ? 'selected=selected': '')+'>I: '+service.institutions_fields[j].name_desc+'</option>';
            }
            search_form += '</select>';
            ...
            search_form += '<p class="institution_display_item"><select name="filter-institutions-field-display" id="filter-institutions-field-display">';
            for (var j in service.institutions_fields) {
                search_form += '<option value="'+service.institutions_fields[j].id+'_i" '+(j == 2 ? 'selected=selected': '')+'>I: '+service.institutions_fields[j].name_desc+'</option>';
            }
            search_form += '</select>';
            search_form += ' <button data-op="+" class="btn_click_filter_display">+</button> <button data-op="-" class="btn_click_filter_display">-</button></p>';
            ...
            search_form += '<p class="institution_display_item"><select name="filter-institutions-field-display" id="filter-institutions-field-display">';
            for (var j in service.institutions_fields) {
                search_form += '<option value="'+service.institutions_fields[j].id+'_i" '+(j == 14 ? 'selected=selected': '')+'>I: '+service.institutions_fields[j].name_desc+'</option>';
            }
            search_form += '</select>';
            search_form += ' <button data-op="+" class="btn_click_filter_display">+</button> <button data-op="-" class="btn_click_filter_display">-</button></p>';
            ...
            search_form += '<p class="institution_display_item"><select name="filter-institutions-field-display" id="filter-institutions-field-display">';
            for (var j in service.institutions_fields) {
                search_form += '<option value="'+service.institutions_fields[j].id+'_i" '+(j == 40 ? 'selected=selected': '')+'>I: '+service.institutions_fields[j].name_desc+'</option>';
            }
            search_form += '</select>';
            search_form += ' <button data-op="+" class="btn_click_filter_display">+</button> <button data-op="-" class="btn_click_filter_display">-</button></p>';
            ...
            search_form += '<p class="institution_sort_item"><select name="filter-institutions-field-sort" id="filter-institutions-field-sort">';
            for (var j in service.institutions_fields) {
                search_form += '<option value="'+service.institutions_fields[j].id+'_i" '+(j == 40 ? 'selected=selected': '')+'>I: '+service.institutions_fields[j].name_desc+'</option>';
            }
            search_form += '</select>';
            ...
            <p style="text-align: center;"><button id="filter-institutions-button">Search</button></p>\
            </div>';

			$('.ui-layout-center').append(search_form);
			$('#filter-institutions-dialog').dialog({
				modal: true,
				height: 'auto',
				width: 'auto'
			});
			...
			$('#filter-institutions-input').keypress(function (e) {
			  if (e.which == 13) {
			    $('#filter-institutions-button').click();
			    return false;
			  }
			});

			$('#filter-institutions-button').off('click').on('click', function() {
				var matchandor = $("input[name=filter_andor]:checked").val();
				//console.log( 'match: ' + matchandor );

                var display_fields = [];
                $('select[name=filter-institutions-field-display]').each( function() {
                  var field_id = $(this).val();
                  display_fields.push( field_id.split('_') );
                });

                var sort_fields = [];
                $('select[name=filter-institutions-field-sort]').each( function() {
                  var field_id = $(this).val();
                  sort_fields.push( field_id.split('_') );
                });

				var filters = [];
				$('.institution_filter_item').each( function() {
  				  var keyword = $(this).find('#filter-institutions-input').val();
	  			  var type = $(this).find('#filter-institutions-filter-type').val();
				  var field = $(this).find('#filter-institutions-field').val();
				  filters.push([ field.split('_'), type, keyword ]);
				});

				let inst, inst_fields, results = [], cvalue, collect = false, collect_test = false;
				for ( var i in service.institutions ) {
				  inst = service.institutions[i];
				  if (inst['status'] != 'active') { continue; } // only check active institutions
				  inst_fields = inst.fields;
  				...
  				for ( var j = 0; j < filters.length; j++ ) {
  					...
  					if ( filters[j][0][1] === 'i' ) {
					  	cvalue = inst_fields[ filters[j][0][0] ];
					  	filters[j][0].push( 'I: ' + service.institutions_fields[ filters[j][0][0] ].name_desc );
							if ( service.institutions_fields[ filters[j][0][0] ].name_desc === 'Join date' ) {
								cvalue = inst['join_date'];
							}
						} else {
					  	if ( matchandor === "0" ) { break; }
					  	else { continue; }
                ...
                // sort results
                var sorted_institutions = [], sorted_item;
                for ( var i = 0, ilen = results.length; i < ilen; i++ ) {
                  sorted_item = [ results[i] ];
                  inst_fields = service.institutions[ results[i] ] ? service.institutions[ results[i] ].fields : null;
                  for ( var j = 0, jlen = sort_fields.length; j < jlen; j++ ) {
										if ( inst_fields && sort_fields[j][1] === 'i' ) {
                      sorted_item.push( String( inst_fields[ sort_fields[j][0] ] ) );
                    }
                  }
                  sorted_institutions.push( sorted_item );
                }


                sorted_institutions.sort( function (a,b) {
                  for ( var i = 1, ilen = a.length; i < ilen; i++ ) {
                      if ( a[i].toLowerCase() < b[i].toLowerCase() ) { return -1; }
                      else if ( a[i].toLowerCase() > b[i].toLowerCase() ) { return 1; }
                  }
                ...
                results = [];
                for ( var i = 0, ilen = sorted_institutions.length; i < ilen; i++ ) {
                    results.push( sorted_institutions[i][0] );
                }

				// process results:

				var label = 'Filtered results: institutions';
				var tabid = service.addTab(label);
				$('#tabs').tabs('option', 'active', $('#'+tabid+'Selector').index());

                var header = [ {"bVisible": false} ];
                for ( var hi = 0, hilen = display_fields.length; hi < hilen; hi++ ) {
									if ( display_fields[hi][1] === 'i' ) {
                    header.push( { "sTitle": service.institutions_fields[ display_fields[hi][0] ].name_desc, "sClass": "td_align_center" } );
                  }
                }
                ...
                for( var m = 0; m < results.length; m++ ) {
                  ...
                  for ( hi = 0, hilen = display_fields.length; hi < hilen; hi++ ) {
										if ( display_fields[hi][1] === 'i' ) {
											var options = service.find_field_options_by_id_institutions( display_fields[hi][0] );
											var tmpdata = service.institutions[ results[m] ].fields[ display_fields[hi][0] ];
											if ( options[ tmpdata] ) { tmpdata = options[ tmpdata ]; }
	                  	mdataarr.push( tmpdata );
										}
                  }
  	              for ( var hii = 0; hii < mdataarr.length; hii++ ) {
	                if ( mdataarr[hii] === undefined || mdataarr[hii] === null ) { mdataarr[hii] = ''; }
			  ...
			  if ( mdata.length > 0 ) {
                ...
                $('<button id="export-xls-filtered-' + service.tabCounter + '" style="margin-right: 20px;"><img src="images/icons/excel.png" border=0 style="vertical-align: middle;"> Export to Excel</button>')
                  .prependTo('#dtmem-' + service.tabCounter + '-tabs-' + service.tabCounter + '_filter')
                  .on('click', function() {
                    var wb = new Workbook(), ws = sheet_from_array_of_arrays(mdata), ws_name = "ePIC-institutions-";
                    wb.SheetNames.push(ws_name);
                    wb.Sheets[ws_name] = ws;
                    var wbout = XLSX.write(wb, {bookType:'xlsx', bookSST:true, type: 'binary'});
                    saveAs( new Blob([s2ab(wbout)],{type:"application/octet-stream"}), ws_name+( Date.now() / 1000 | 0 )+".xlsx" );
...
*/
				...
				$('#filter-institutions-dialog').dialog("close");
			});
	},
		...
		if ($("#search-members-dialog").length > 0) {
			...
			$('#search-members-button').click(function() {
				...
				$.getJSON(service.service_url+'?q=/service/search/object:members/type:'+type+'/field:'+field+'/keyword:'+encodeURIComponent(keyword)+'/autocomplete:no/date:'+Date.now(), function( data, status, xhr ) {
					...
					if (length <= 0) {
						service.display_notification_dialog("NO members found!<br><br>You have typed: <b>"+keyword.replace(/\W/g, '')+'</b>');
					} else if (length == 1) {
	...
	display_search_institutions: function() {
		...
		$('#search-institutions-button').click(function() {
			...
			$.getJSON(service.service_url+'?q=/service/search/object:institutions/type:'+type+'/field:'+field+'/keyword:'+encodeURIComponent(keyword)+'/autocomplete:no/date:'+Date.now(), function( data, status, xhr ) {
				...
				if (length <= 0) {
					service.display_notification_dialog("NO institutions found!<br><br>You have typed: <b>"+keyword.replace(/\W/g, '')+'</b>');
				} else if (length == 1) {
		...
		$('#edit-institution-'+service.tabCounter+'-'+id).button()
			.click(function( event ) {
				...
				$( "#edit-institution-dialog" ).dialog({
					...
					buttons: {
                        "Geocode": function() {
							var iname = $('[name="instedit-name_full"]').val().trim() || "",
								icountry = $('[name="instedit-country"]').val().trim() || "",
								ilat = $('[name="instedit-geo_lattitude"]').val().trim() || "",
								ilon = $('[name="instedit-geo_longitude"]').val().trim() || "";
							var req = [];
							if ( iname && iname.length ) { req.push( iname ); }
							if ( icountry && icountry.length ) { req.push( icountry ); }
                            ...
                            if (res == undefined) {
                                ...
                                service.display_geocode_dialog([res.lat, res.lon], res.desc, req, ilat, ilon );
                            }
                        },
	...
	display_board: function() {
		var service = this;
		var label = 'ePIC Institutional Board';
		var tabid = service.addTab(label);
		$('#tabs').tabs('option', 'active', $('#'+tabid+'Selector').index());
		var header = [ {"bVisible": false},
	...
	display_members:function(id) {
		...
		for (var i in this.members) {
            ...
            if ( field[89] != undefined ) {
								var tmp_opts = this.members_fields["89"]["options"].split(',');
								for ( var tmp_i = 0, tmp_i_len = tmp_opts.length; tmp_i < tmp_i_len; tmp_i++) {
									var tmp_j = tmp_opts[tmp_i].split(':');
									if ( field[89] == tmp_j[0] ) { physicist = tmp_j[1]; break; }
								}
            }
  ...
  confirm_mass_emailr: function() {
    ...
    alert( 'WARNING, found representatives without emails: ' + check.no_email_rep.join(', ') );
      //return;
    }
    if ( check.no_rep_inst.length > 0 ) {
    alert( 'NOTE: found institutions without representatives (emails will not be sent): ' + check.no_rep_inst.join(', ') );
      //return;
    }
    var r = confirm("Do you want to send out notifications to IB Representatives, containing PhoneBook information?");
    if (r == true) {
      $.ajax({
